...
def load_data():
    ...
    df['Average Cost for two'] = df['Average Cost for two'].astype(
        str).str.replace(',', '')
    ...
    necessary_cols = [
        'Restaurant Name', 'City', 'Cuisines', 'Has Table booking',
    ...
    # Normalize features
    scaler = MinMaxScaler()
    df['Cost_single_person_norm'] = scaler.fit_transform(
        df[['Cost_single_person']])
...
def get_recommendations(user_preferences,
    ...
    # Ensure we only keep top_n recommendations after filtering
    recommendations = recommendations.head(top_n)

    # Now add similarity score - make sure lengths match
    recommendations['similarity_score'] = sim_scores[
        restaurant_indices][:len(recommendations)]
...
def index():
    if request.method == 'POST':
        ...
        user_prefs = {
            ...
            'cuisines': ' '.join(request.form.getlist('cuisines')),
        ...
        rec_list = recommendations[[
            ...
            'Aggregate rating', 'Address', 'similarity_score'
        ]].to_dict('records')
